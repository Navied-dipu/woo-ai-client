            ...
            {promptsList.length === 0 ? (
              ...
              promptsList.map((prompt, index) => (
                <tr key={prompt._id || index} className="border-t hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`px-3 py-1 rounded-full text-sm ${prompt.status === "approved"
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center gap-3 text-xl">
                      {/* VIEW BUTTON */}
                      <Link href={`/allprompts/${prompt._id || prompt.id}`} title="View Prompt" className="text-blue-500 hover:text-blue-700">
                        👁️
                      </Link>

                      {/* EDIT BUTTON */}
                      <Link href={`/dashboard/creator/my-prompts/${prompt._id || prompt.id}`} title="Edit Prompt" className="text-green-500 hover:text-green-700">
                        ✏️
                      </Link>
